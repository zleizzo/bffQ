...
def compute_batch_F(batch, Q):
    ...
    for experience in batch:
        # batch = deque of length n
        # batch[i] = (s_{m+i}, a_{m+i}, rwd_{m+i}, done_{m+i})
        cur_s = experience[0][0]
        cur_a = experience[0][1]
        nxt_s = experience[1][0]
        ...
        for m in range(n):
            ds    = experience[m + 2][0] - experience[m + 1][0] # Compute \Delta s
            new_s = cur_s + ds
