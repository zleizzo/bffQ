...
def PD(T, batch_size, beta0, eta0, e = 0.1, Q = Net(), y = Net(), trueQgraph = None, adaptive = True):
    ...
    cur_a = e_greedy2(Q(map_to_input(cur_s)), e)
    
    nxt_s = transition(cur_s, cur_a)
    nxt_a = e_greedy2(Q(map_to_input(nxt_s)), e)
...
plt.legend()
plt.savefig(f'plots/nn_q_pd_control_{experiment_n}.png')
...
plt.legend()
plt.savefig(f'plots/nn_error_pd_control_{experiment_n}.png')
...
with open(f'csvs/control/q_pd_{experiment_n}.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    for row in pd.numpy():
        writer.writerow(row)
...
with open(f'csvs/control/error_pd_{experiment_n}.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
