...
def uniform_SGD(Q_init = np.zeros((N, 2)), batch_size=100, T=50000000):
    Q = Q_init.copy()
...
def uniform_BFFQ(Q_init = np.zeros((N, 2)), batch_size=100, T=50000000):
    Q = Q_init.copy()
    errors  = np.zeros(int(T / batch_size))
    bellman = np.zeros(int(T / batch_size))
...
# Run experiment
###############################################################################
Q_unif, errors_unif, bellman_unif = uniform_SGD(Q_init = np.zeros((N, 2)), batch_size=100, T=10000000)
