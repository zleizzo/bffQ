...
Q_nBFF, e_nBFF = nBFF(n, T, learning_rate, batch_size, Net(), true)
...
plt.legend()
plt.savefig(f'plots/eval_error_{n}bff.png')


with open(f'csvs/nn_eval/q_{n}bff.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    for row in nbff.numpy():
        writer.writerow(row)

with open(f'csvs/nn_eval/error_{n}bff.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
